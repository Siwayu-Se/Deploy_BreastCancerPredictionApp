...
if st.button("ทำนายผล"):
    ...
    confidence = np.max(proba) * 100  # แปลงเป็นเปอร์เซ็นต์

    # แสดงผลลัพธ์พร้อมความมั่นใจ
    if prediction == 1:
        st.error(f"💡 ผลการทำนาย: ผู้ป่วยมีแนวโน้มเป็นมะเร็ง (Malignant)\n\n🔎 ความมั่นใจของโมเดล: {confidence:.2f}%")
    else:
        st.success(f"✅ ผลการทำนาย: ผู้ป่วยไม่มีแนวโน้มเป็นมะเร็ง (Benign)\n\n🔎 ความมั่นใจของโมเดล: {confidence:.2f}%")
